...
public class DBV extends View {
    ...
    public DBV(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DBV(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public DBV(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public Bitmap getMaskBitmap() {
        ...
        if (maskBitmap != null) {
            ...
            Canvas canvas = new Canvas(bitmap);
            canvas.drawBitmap(maskBitmap, new Matrix(), null);
        }
        return bitmap;
    }
    ...
    public Bitmap getCutBitmap() {
        ...
        if (originBitmap != null) {
            ...
            canvas.drawBitmap(maskBitmap, new Matrix(), cutPaint);

            // 使用遮罩层裁剪图像
            cutPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));
            canvas.drawBitmap(originBitmap, new Matrix(), cutPaint);

            cutPaint.setXfermode(null);
        }
        return bitmap;
    }
    ...
    private synchronized void updateImageBitmap() {
        if (originBitmap == null) {//如果原始图片为空，则创建一个和控件视图宽高一致的图片
            ...
            deleteBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
            deleteCanvas = new Canvas(previewBitmap);
            ...
            previewCanvas = new Canvas(previewBitmap);
            deleteBitmap = Bitmap.createBitmap(bitmapWidth, bitmapHeight, Bitmap.Config.ARGB_8888);
    ...
    public void clear() {
        ...
        deleteCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        drawList.clear();
        backupDrawList.clear();
        hasUndo = false;
        hasRedo = false;
    ...
    public void undo(int index) {
        if (!drawList.isEmpty() && index < drawList.size() && index != -1) {
            maskCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
            drawingCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
            deleteCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
            //删除最后一步
            ...
            for (Draw draw : drawList) {
                //mask图层换画笔颜色
                draw.setPaint(createPaint(drawMode, Color.WHITE, draw.getPaint().getStrokeWidth()));
                draw.draw(maskCanvas);
                //进行绘制
                draw.setPaint(createPaint(drawMode, paintColor, draw.getPaint().getStrokeWidth()));
                draw.draw(drawingCanvas);
                draw.actionDelete(deleteCanvas);
            }
            invalidate();
            isRevoked = true;
            hasRedo = true;
    ...
    public void draw(@NonNull Draw draw) {
        if (drawingCanvas != null) {
            if (draw.getPaint() == null) {//如果画笔为空则根据当前配置自动创建画笔，如果draw 是绘制其他有依赖的需自己配置后再传进来
                draw.setPaint(createPaint(drawMode, paintColor, 0));
            }
            //mask图层换画笔颜色
            draw.setPaint(createPaint(drawMode, Color.WHITE, draw.getPaint().getStrokeWidth()));
            draw.draw(maskCanvas);
    ...
    private void drawBitmap(Canvas canvas, Matrix matrix, boolean isTouch) {
        ...
        if (deleteBitmap != null) {
            canvas.drawBitmap(deleteBitmap, matrix, null);
        }
    ...
    private void drawTouchPoint(Canvas canvas, float x, float y, float radius) {
        canvas.drawCircle(x, y, radius * bitmapRatio, pointPaint);
    }
     ...
     * @return
     */
    private Paint createPaint(int drawMode, @ColorInt int color, float paintSize) {
        Paint paint = new Paint();
        if (drawMode == DrawMode.ERASER) { // 当为橡皮擦模式时
            paint.setStyle(Paint.Style.STROKE);
            paint.setColor(Color.BLACK);
            paint.setStrokeWidth(eraserStrokeWidth * bitmapRatio);
            ...
            if (paintSize<=0){
                paint.setStrokeWidth(lineStrokeWidth * bitmapRatio);
            }else{
    ...
    private Draw createDraw(@DrawMode int drawMode) {
        ...
        if (drawClass != null) {
            try {
                ...
                this.drawBitmap = Bitmap.createScaledBitmap(drawBitmap, (int) (screenWidth / 16 * bitmapRatio), (int) (screenWidth / 16 * bitmapRatio), true);

                draw.setBitmap(drawBitmap);
                draw.setMaxX(drawingBitmap.getWidth());
                draw.setMaxY(drawingBitmap.getHeight());
...
//                    draw.setBitmap(drawBitmap);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                bitmapRatio = drawingBitmap.getWidth() / screenWidth;
                if (bitmapRatio > 5) bitmapRatio = 5;
                if (isDrawEnabled && event.getPointerCount() == 1) {//单指
                    isZoom = false;
                    isDraw = false;
                    paint = createPaint(drawMode, paintColor, 0);
            ...
            case MotionEvent.ACTION_MOVE:
                if (isDrawEnabled && !isZoom) {
                    ...
                    if (Math.abs(lastX - x) > touchTolerance || Math.abs(lastY - y) > touchTolerance) {
                        ...
                        if (ratioY > (drawingBitmap.getHeight())) {
                            ratioY = drawingBitmap.getHeight();
                        }

                        if (ratioX < 0) {
                            ratioX = 0;
                        }
                        if (ratioX > (drawingBitmap.getWidth())) {
                            ratioX = drawingBitmap.getWidth();
                        }
            ...
            case MotionEvent.ACTION_UP:
                float x = event.getX();
                float y = event.getY();
                int absMx = (int) Math.abs(x - mDownX);
                int absMy = (int) Math.abs(y - mDownY);
                if (absMx < TOUCH_TOLERANCE && absMy < TOUCH_TOLERANCE) {
                    if (onSingleClickListener != null) {
                        float ratioX = (x - currentTranslateX) / currentRatio;
                        float ratioY = (y - currentTranslateY) / currentRatio;
                        onSingleClickListener.OnSingleClick(ratioX, ratioY, bitmapRatio);
                    }
                }
     ...
     * @return
     */
    public LinkedList<Draw> getListDraw() {
        return drawList;
    }
     ...
     * @param drawBitmap
     */
    public void setDrawBitmap(Bitmap drawBitmap, Activity context) {
    ...
    public interface OnSingleClickListener {
        void OnSingleClick(float x, float y, float bitmapRatio);
    }
