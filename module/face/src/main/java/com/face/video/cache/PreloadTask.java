...
public class PreloadTask implements Runnable {
    ...
    public final static List<String> blackList = new ArrayList<>();
    ...
    private void start() {
        ...
        try {
            ...
            connection.setRequestProperty("Range", "bytes=0-" + PreloadManager.PRELOAD_LENGTH);
            ...
            while ((length = in.read(bytes)) != -1) {
                ...
                if (mIsCanceled || read >= PreloadManager.PRELOAD_LENGTH) {
                    if (mIsCanceled) {
                        LogUtils.d("预加载取消：" + mPosition + " 读取数据：" + read + " Byte");
                    } else {
                        LogUtils.d("预加载成功：" + mPosition + " 读取数据：" + read + " Byte");
                    }
                    break;
                }
            }
        } catch (Exception e) {
            LogUtils.d("预加载异常：" + mPosition + " 异常信息：" + e.getMessage());
            // 关入小黑屋
