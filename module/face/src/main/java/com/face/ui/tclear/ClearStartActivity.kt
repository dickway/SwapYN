...
import com.face.R
import com.face.BR
import com.face.databinding.ActivityStartClearBinding
...
class ClearStartActivity : BaseBindingActivity<ActivityStartClearBinding, NullViewModel>(
    R.layout.activity_start_clear,
    NullViewModel::class.java
) {
    override fun init(savedInstanceState: Bundle?) {

        mBinding?.apply {
            videoView.setVideoPath("android.resource://$packageName/${com.key.R.raw.clear2}")
            videoView.seekTo(10)
    ...
    fun toRecord() {
        GVM.INSTANT.payPage.value = "Home_tool_clear_record"
        if (!GVM.INSTANT.isVip.value && !SPUtils.toolUse) {
            VipActivity.jump(this@ClearStartActivity)
            return
        }
        openActivity<ToolPicHistoryActivity> {
            putString("aiTaskType", AiTaskType.LAMA_CLEANER)
        }
    }

    fun selectorPhoto() {
        GVM.INSTANT.payPage.value = "Home_tool_clear"
        if (!GVM.INSTANT.isVip.value&& !SPUtils.toolUse) {
            VipActivity.jump(this@ClearStartActivity)
            return
        }
                    ...
                    if (result.size > 0) {
                        result[0]?.apply {
                            if (availablePath.isNotEmpty()) {
                                fragment.openActivity<ClearAiActivity> {
