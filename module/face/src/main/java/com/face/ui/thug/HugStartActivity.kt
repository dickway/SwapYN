...
class HugStartActivity : BaseBindingActivity<ActivityStartHugBinding, HugStartViewModel>(
    R.layout.activity_start_hug,
    HugStartViewModel::class.java
) {
    var tagVideo = ""
    val voideUrl = SPUtils.introductionVideo
        ...
        ?: "https://d5z47om850v8h.cloudfront.net/admin/2025-08-25/onehug.mp4"

    override fun init(savedInstanceState: Bundle?) {
        mBinding?.apply {
            tagVideo = mBinding?.videoView?.getVideoKey() ?: ""
            mBinding?.videoView?.loadBgImage1(com.key.R.mipmap.img_start_hug)
...
//        }
    ...
    fun toRecord() {
        if (!GVM.INSTANT.isVip.value) {
            ...
            VipActivity.jump(this@HugStartActivity)
            return
        }
