...
import com.face.R
import com.face.BR
import com.face.databinding.ActivityStartLiveBinding
...
import com.face.viewmodel.activity.LiveStartViewModel
...
class LiveStartActivity : BaseBindingActivity<ActivityStartLiveBinding, LiveStartViewModel>(
    R.layout.activity_start_live,
    LiveStartViewModel::class.java
) {
    override fun init(savedInstanceState: Bundle?) {
        mBinding?.apply {
            videoView.setVideoPath("android.resource://$packageName/${com.key.R.raw.live}")
            videoView.seekTo(10)
    ...
    fun toRecord() {
        GVM.INSTANT.payPage.value = "Home_tool_live_record"
        if (!GVM.INSTANT.isVip.value) {
            VipActivity.jump(this@LiveStartActivity)
            return
        }
        openActivity<FunZoneHistoryActivity> {
            putString("aiTaskType", AiTaskType.DYNAMIC)
        }
    }

    fun onstart() {
        GVM.INSTANT.payPage.value = "Home_tool_live"
        if (!GVM.INSTANT.isVip.value) {
            VipActivity.jump(this@LiveStartActivity)
            return
        }
                    ...
                    if (result.size > 0) {
                        result[0]?.apply {
                            if (availablePath.isNotEmpty()) {
                                fragment.openActivity<LiveAiActivity> {
