...
class CutoutStartActivity : BaseBindingActivity<ActivityStartCutoutBinding, CutoutStartViewModel>(
    R.layout.activity_start_cutout,
    CutoutStartViewModel::class.java
) {
    override fun init(savedInstanceState: Bundle?) {
        SPUtils.isMaterial = false

        mBinding?.apply {
            videoView.setVideoPath("android.resource://$packageName/${com.key.R.raw.delete}")
            videoView.seekTo(10)
    ...
    fun toRecord() {
        GVM.INSTANT.payPage.value = "Home_tool_background_record"
        if (!GVM.INSTANT.isVip.value&& !SPUtils.toolUse) {
            VipActivity.jump(this@CutoutStartActivity)
            return
        }
    ...
    fun selectorPhoto() {
        GVM.INSTANT.payPage.value = "Home_tool_background"
        if (!GVM.INSTANT.isVip.value&& !SPUtils.toolUse) {
            VipActivity.jump(this@CutoutStartActivity)
            return
        }
                    ...
                    if (result.size > 0) {
                        result[0]?.apply {
                            if (availablePath.isNotEmpty()) {
                                fragment.openActivity<CutoutAiActivity> {
