...
class KissActivity : BaseBindingActivity<ActivityKissBinding, KissViewModel>(
    R.layout.activity_kiss,
    KissViewModel::class.java
) {
        ...
        mModel.taskId.observe(this) {
            if (it.isNotEmpty()) {
                ...
                openActivity<ToolGeneratingActivity> {
                    putString("taskId", it)
                    putString("taskType",  AiTaskType.IMG2KISS)
                }
            }
        }
    ...
    fun postSend() {
        ...
        mModel.firstImg=""
        if (GVM.INSTANT.userInfo.value.tflops < mModel.kissPinot.value.toInt()) {
            GVM.INSTANT.payPage.value="Buy_AIPoints_Kiss"
            ...
            BaseRedDialog(
                ...
                onBtnOK = {
                    ...
                    val filePath = if (mModel.isSingular.value) mModel.img3.value else mModel.img1.value
    ...
    fun selectorPhoto(index: Int) {
        ...
        if (!GVM.INSTANT.isVip.value) {
            VipActivity.jump( this@KissActivity)
            return
        }
