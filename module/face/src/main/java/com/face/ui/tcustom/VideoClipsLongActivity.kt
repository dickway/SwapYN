...
import com.face.databinding.ActivityVideoClipsLongBinding
import com.face.util.GVM
import com.face.util.SPUtils
import com.face.videoclips.interfaces.VideoTrimListener
import com.face.viewmodel.activity.VideoClipsLongModel
...
class VideoClipsLongActivity :
    BaseBindingActivity<ActivityVideoClipsLongBinding, VideoClipsLongModel>(
        R.layout.activity_video_clips_long,
        VideoClipsLongModel::class.java
    ), VideoTrimListener {
    ...
    override fun init(savedInstanceState: Bundle?) {
        ...
        try {
            var minShoot: Long = 2 * 1000L
            var maxShoot: Long = 60 * 1000L
            if (paramType.contains("300")) {
                minShoot = 20 * 1000L
                maxShoot = 300 * 1000L
            }else if(paramType.contains("180")){
                minShoot = 20 * 1000L
                maxShoot = 180 * 1000L
            }
            mBinding?.apply {
                trimmerView.initConfig(minShoot, maxShoot)
                trimmerView.setOnTrimVideoListener(this@VideoClipsLongActivity)
