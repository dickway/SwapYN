...
import com.a.databinding.ActivityAtoolStartcartoonBinding
import com.a.viewmodel.ACartoonStartViewModel
...
class ACartoonStartActivity :
    BaseBindingActivity<ActivityAtoolStartcartoonBinding, ACartoonStartViewModel>(
        R.layout.activity_atool_startcartoon,
        ACartoonStartViewModel::class.java
    ) {
    override fun init(savedInstanceState: Bundle?) {
        mBinding?.apply {
            videoView.setVideoPath("android.resource://$packageName/${com.key.R.raw.tool3}")
            videoView.seekTo(10)
    ...
    fun selectorPhoto() {
        GVM.INSTANT.payPage.value = "AHome_tool_cartoon"
                    ...
                    if (result.isNotEmpty()) {
                        result[0]?.apply {
                            if (availablePath.isNotEmpty()) {
                                frament.openActivity<ACartoonActivity> {
    ...
    fun toRecord() {
        GVM.INSTANT.payPage.value = "AHome_tool_cartoon_record"
        if (!GVM.INSTANT.isVip.value&& !SPUtils.toolUse) {
            openActivity<AVipActivity>()
            return
        }
