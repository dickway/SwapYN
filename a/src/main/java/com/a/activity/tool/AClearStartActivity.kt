...
class AClearStartActivity : BaseBindingActivity<ActivityAtoolStartclearBinding, NullViewModel>(
    R.layout.activity_atool_startclear,
    NullViewModel::class.java
) {
    override fun init(savedInstanceState: Bundle?) {

        mBinding?.apply {
            videoView.setVideoPath("android.resource://$packageName/${com.key.R.raw.tool4}")
            videoView.seekTo(10)
    ...
    fun toRecord() {
        GVM.INSTANT.payPage.value = "AHome_tool_clear_record"
                    ...
                    if (result.size > 0) {
                        result[0]?.apply {
                            if (availablePath.isNotEmpty()) {
                                frament.openActivity<AClearAiActivity> {
