...
class ALoginAccountActivity :
    BaseBindingActivity<ActivityAloginAccountBinding, ALoginAccountViewModel>(
        R.layout.activity_alogin_account,
        ALoginAccountViewModel::class.java
    ) {

    val checked = MutableLiveData(false)
    override fun init(savedInstanceState: Bundle?) {
        EventUtil.inPage("login_aaccount_activity")
//        mModel.enableLogin.observe(this, Observer {
//            mBinding?.login?.alpha = if (it) 1f else 0.1f
//        })
    ...
    fun onCreateAccount() {
        openActivity<ACreateAccountActivity>()
    }
